...
const Signup = () => {
    ...
    const handleSignup = async (e) => {
        if (user.password === undefined || user.password === "") {
            ...
            toggleunsamePasswordRef()
        } else if (user.password === user.confirmPassword) {
            try {
                delete user.confirmPassword
                const response = await fetch(`${process.env.NEXT_PUBLIC_FETCH_URI}/signup`, {
                    method: "POST",
                    headers: {
            ...
            <div className="bg-grey-lighter flex flex-col mt-6 mb-9">
                <div className="container max-w-sm mx-auto flex-1 flex flex-col items-center justify-center px-2">
                    <div className="bg-white px-6 py-8 rounded shadow-md text-black w-full">
                        ...
                        <button
                            type="submit"
                            className="w-full text-center text-lg py-3 rounded bg-indigo-600 text-gray-200 hover:bg-indigo-800 hover:text-white focus:outline-none my-1" onClick={handleSignup}
                        >Signup</button>
                    </div>
