...
const LHC = ({ params }) => {
  const id = params.id
  if(id > 10)
    throw new Error("Invalid Lab Number")
    ...
    <section className="text-gray-600 body-font relative">
      ...
      <div className="container px-5 py-24 mx-auto flex">
        <div className="lg:w-1/3 md:w-1/2 bg-white rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0 relative z-10 shadow-md">
          <h2 className="text-gray-900 text-3xl mb-1 font-medium title-font">{"Lab " + id}</h2>
          ...
          <button onClick={checkLabs} className="text-white bg-[#480cfc] border-0 py-2 px-6 focus:outline-none hover:bg-[#3a0aca] rounded text-lg transition-all transform hover:scale-105">Check Availability</button>
        </div>
      </div>
    </section>
  )
