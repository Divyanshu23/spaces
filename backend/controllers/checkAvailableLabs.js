...
//   try {
//     // var [rows, fields] = await pool.query(`SELECT lab, capacity, OS, booking_rate FROM labs WHERE capacity >= ? AND lab NOT IN (SELECT DISTINCT lab FROM lab_bookings WHERE date = ? AND start < ? AND end > ?)`, [cap, date, end+':00:00', start+':00:00']);
//     var [rows, fields] = await pool.query(`SELECT lab, capacity, OS, booking_rate FROM labs WHERE capacity >= ? AND lab NOT IN (SELECT DISTINCT lab FROM lab_bookings WHERE date = ? AND start < ?AND end > ?)`, [cap, date, end, start]);
