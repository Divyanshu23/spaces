...
const checkLab = async (req, res) => {
  ...
  try {
    let [rows, fields] = await pool.query(`SELECT * FROM labs WHERE lab = ? AND lab NOT IN (SELECT DISTINCT lab FROM lab_bookings WHERE date = ? AND start < ? AND end > ?)`, [lhc, date, end + ':00:00', start + ':00:00']);
